...
SoloSec exits non-zero when it finds Critical or High issues, which is all most
...
SoloSec exits `1` when any Critical or High finding is present. Medium and below
...
- **`scan`** — runs SoloSec on this repository via the local action. Requires
...
## Using SoloSec from another repository

This repository ships a composite action at `action.yml`. Add a step:

```yaml
- name: SoloSec scan
  uses: maltemindedal/solosec@main
  with:
...
| `artifact-name` | `"solosec-report"` | Name of the uploaded artifact. |
...
1. Builds the bundled `Dockerfile` as `solosec:action`.
...
Nothing in SoloSec is GitHub-specific. Any runner that can build a container and
read an exit code will work:

```bash
docker build -t solosec:ci .
docker run --rm --user "$(id -u):$(id -g)" -v "$PWD:/src" solosec:ci
```

The non-zero exit fails the job. For DAST, add the socket mount and
`SOLOSEC_HOST_WORKSPACE` as described in
